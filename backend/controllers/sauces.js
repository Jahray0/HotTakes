...
exports.createSauce = (req, res, next) => {
    ...
    if(checkedSave) {
        ...
        sauce.save()
            ...
            .catch(error => res.status(400).json({ error: `Erreur 400 Bad request` }));
    };
};
...
exports.modifySauce = (req, res, next) => {
    ...
    if(checkedSave) {
        ...
        res.status(401).json({ message: 'Présence de caractères non autorisés'});
    };
};
...
exports.deleteSauce = (req, res, next) => {
    Sauce.findOne({ _id: req.params.id })
        ...
        .catch(error => res.status(500).json({ error: `La requete n'a pas pu etre traité ` }));
};
